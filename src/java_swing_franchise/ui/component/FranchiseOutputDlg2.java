...
public class FranchiseOutputDlg2 extends JDialog implements ActionListener {
	...
	public void setProduct(ArrayList<Product> pro) {
		...
		Comparator<Product> test = new Comparator<Product>() {
			...
			public int compare(Product o1, Product o2) {
				return o2.getProfitPrice() - o1.getProfitPrice();
			}
		};
		
		Collections.sort(pro, test); // 마진액으로 정렬
		
		int lank = 1;
		int num = 1;
		for (int i = 0; i < pro.size(); i++) {
			if (num < pro.size()) {
				if (pro.get(i).getProfitPrice() == pro.get(num).getProfitPrice()){
